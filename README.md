...
- 频谱覆盖 C1–C9（约 33 Hz–8.37 kHz），色标为 −90 至 0 dBFS；支持冻结、回放与缩放，可通过底部切换按钮切回音高曲线。右上角齿轮按钮直接进入设置。
...
- 选择已有录音或导入 WAV 后，在后台分析整段文件，无需播放即可切换查看完整音高曲线或 FFT 频谱。时间轴覆盖 0 至文件结尾，播放时显示位置线；暂停、停止和播放结束后保留总览，开始监听则恢复实时滚动。
- PCM16 单/双声道 WAV 输入，8–192 kHz，导入后统一为单声道；设备采集保留原始采样率。
- 视觉节拍器、BPM、拍号、节拍线、缩放、噪声阈值、平滑、颜色和快捷按钮设置。
- 竖屏、横屏、平板及系统大字体布局；冻结图表和手动调整音域。
- 音高曲线与 FFT 频谱仅通过手势调整音域：双指竖直拉开放大、捏合缩小，上下拖动平移；手动调整会暂停自动跟随。双击主显示区域可自动适配当前曲线或频谱的音域并恢复跟随，单击冻结、再次单击解冻。
- 后台或系统音频中断时释放麦克风、结束并保存录音；回放保留暂停位置。正常切回前台后自动恢复音高监听，无需手动开始；录音需再次点击录音按钮。
...
```sh
# Android APK（仅 arm64；当前 release 使用开发调试签名；发布前配置自己的签名）
...
Android release 的 APK/AAB 仅包含 `arm64-v8a` 原生库，不支持 32 位 ARM 或 x86_64 设备；
Gradle 配置会强制此限制，`--target-platform android-arm64` 另外避免编译其他架构。
debug/profile 保留 Flutter 的默认架构支持，供真机与模拟器开发使用。
